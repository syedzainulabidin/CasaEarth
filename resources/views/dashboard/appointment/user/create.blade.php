            ...
            therapistSelect.addEventListener('change', function() {
                ...
                fetch(`/dashboard/appointment/${therapistId}/availability`)
                    ...
                    .then(data => {
                        ...
                        calendar = flatpickr(dateInput, {
                            ...
                            enable: [
                                function(date) {
                                    ...
                                    const dayName = Object.keys(dayMap).find(k => dayMap[
                                        k] === weekday);
                            ...
                            onChange: function(selectedDates) {
                                ...
                                data.slots.forEach(slot => {
                                    const bookedForDate = Array.isArray(data.booked[
                                        selectedDateStr]) ? data.booked[
                                        selectedDateStr] : [];
                                    if (!bookedForDate.includes(slot)) {
                                        ...
                                        const [start24, end24] = slot.split('-')
                                            .map(s => s.trim());
                                        slotSelect.innerHTML +=
                                            `<option value="${slot}">${formatTo12Hour(start24)} - ${formatTo12Hour(end24)}</option>`;
                                    }
                                });
                        ...
                        document.querySelector('.charges').innerText = `$${data.charges}`;
